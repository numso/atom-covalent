...
firepad.AtomAdapter = AtomAdapter = (function() {
  ...
  function AtomAdapter(atom) {
    this.atom = atom; // this is the editor
    this.onChange = __bind(this.onChange, this);
    this.buffer = this.atom.buffer;
    this.grabDocumentState();
    this.buffer.on('changed', this.onChange);

    this.atom.onDidChangeCursorPosition(function () {
      this.firepadTriggers.cursorActivity();
    }.bind(this));

    this.atom.onDidAddSelection(function () {
      this.firepadTriggers.cursorActivity();
    }.bind(this));
  ...
  AtomAdapter.prototype.detach = function() {
    this.buffer.off('changed');
    this.buffer.off('cursor:moved');
    this.buffer.off('selection:changed');
  };
  ...
  AtomAdapter.prototype.setOtherCursor = function(cursor, color, clientId) {
    atom.drawFakeStuffAt(cursor, color, clientId);
  };
